...
public class CalculatorController {
    ...
    public ResponseEntity<List<LoanOfferDto>> calculateOffers(@RequestBody LoanStatementRequestDto request) {
        logger.info("Входные данные для calculateOffers: {}", request);
        List<LoanOfferDto> offers = calculatorService.calculateOffers(request);
        logger.info("Результат calculateOffers: {}", offers);
    ...
    public ResponseEntity<CreditDto> calculateCredit(@RequestBody ScoringDataDto scoringData) {
        logger.info("Входные данные для calculateCredit: {}", scoringData);
